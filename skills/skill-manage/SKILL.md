---
name: skill-manage
description: 管理 AFreeCoder 的 Skill：用 npx -y skills@latest 安装、更新、卸载自建和第三方 Skill；修改或新建 my-skills 自建 Skill 后完成提交、推送并刷新本机安装。遇到"安装 Skill""更新或卸载 Skill""修改或新建自建 Skill""收藏第三方 Skill"等请求时使用。
---
...
- 安装、更新、卸载一律通过 [skills CLI](https://github.com/vercel-labs/skills)（`npx -y skills@latest`）执行，不手工复制目录。
...
# 安装自建 Skill
npx -y skills@latest add AFreeCoder/my-skills --skill <name> [-g] -y

# 更新 / 卸载
npx -y skills@latest update [<name>] [-g|-p]
npx -y skills@latest remove <name>
```

全局安装不要传 `-a codex`：Codex 的全局目录就是 `~/.agents/skills`，传了反而会写入已弃用的 `~/.codex/skills`。

安装记录（lock）分范围各存一份：全局在 `~/.agents/.skill-lock.json`，项目级在 `<project>/skills-lock.json`（无时间戳、可随项目提交）。没有跨范围的汇总命令——用户想总览已装 Skill 时，执行 `npx -y skills@latest list -g` 和 `npx -y skills@latest list` 两条命令并合并汇报。
...
4. 推送后刷新本机安装。npx 装的是复制的实体，不会自动跟随远程，每个安装位置都要显式跑一次 update——本闭环代为执行，用户无需手动操作：全局安装必刷 `npx -y skills@latest update <name> -g -y`；当前项目也装有该 Skill 时，再执行一次 `npx -y skills@latest update <name> -p -y`。其他项目里的安装本次触达不到，留待下次在该项目中操作 Skill 时刷新。
...
定稿后走上面的闭环（推送，再用 `npx -y skills@latest add AFreeCoder/my-skills --skill <name> -g -y` 恢复正式安装）。
