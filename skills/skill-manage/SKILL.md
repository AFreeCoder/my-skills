...
- 安装、更新、卸载一律通过 [skills CLI](https://github.com/vercel-labs/skills)（`npx skills`）执行，不手工复制目录；`skill_manage.py` 已退役。
...
4. 推送后立即刷新本机安装：`npx skills update <name> -g -y`；当前项目也装有该 Skill 时，再执行一次项目级刷新。
