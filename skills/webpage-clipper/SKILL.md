...
1. 用环境变量 `OBSIDIAN_VAULT` 指向本地知识库根目录，默认输出到其下的 `10_raw`，图片输出到 `assets`；未设置时用 `--out-dir` 显式指定输出目录。
...
```bash
python scripts/clip_webpage.py \
...
- 默认图片目录为知识库根目录下的 `assets`（如需其他路径用 `--assets-dir`）。
...
- `--url`：网页地址（必填）。
- `--out-dir`：Markdown 输出目录（默认 `$OBSIDIAN_VAULT/10_raw`；未设置该环境变量时必填）。
- `--assets-dir`：图片输出目录（默认 `$OBSIDIAN_VAULT/assets`，回退为 `out-dir/assets`）。
- `--title`：标题覆盖。
...
- 通过环境变量 `OBSIDIAN_VAULT` 指定本地知识库根目录；脚本不内置任何默认库路径。
